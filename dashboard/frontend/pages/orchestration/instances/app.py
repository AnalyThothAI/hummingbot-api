...
def archive_bot(bot_name):
    ...
    if stop_response.get("status_code") not in (200, 201):
        st.warning(f"Stop request failed for {bot_name}; continuing with archive.")
    ...
    status_code = remove_response.get("status_code")
    if status_code == 401:
        st.error("Unauthorized. Check BACKEND_API_USERNAME and BACKEND_API_PASSWORD.")
    else:
        st.error(remove_response.get("error", f"Failed to archive bot {bot_name}."))
...
def render_bot_status_card(instance):
    """Render a status-only card for bots that are not fully online."""
    ...
    with st.container(border=True):
        if health_state == "degraded":
            st.warning(f"🤖 **{bot_name}** - Running (MQTT Disconnected)")
        elif health_state == "stopped":
            st.error(f"🤖 **{bot_name}** - Stopped")
        elif health_state == "orphaned":
            st.warning(f"🤖 **{bot_name}** - Orphaned (MQTT Only)")
        else:
            st.info(f"🤖 **{bot_name}** - Status Unknown")

        st.caption(f"Docker: {docker_status} | MQTT: {mqtt_status}")
        if reason:
            st.caption(f"Reason: {reason_map.get(reason, reason)}")
...
def render_bot_card(bot_name, instance=None):
    """Render a bot performance card using native Streamlit components."""
    docker_status = instance.get("docker_status", "unknown") if instance else "unknown"
    mqtt_status = instance.get("mqtt_status", "unknown") if instance else "unknown"
    reason = instance.get("reason") if instance else None
    reason_map = {
        "mqtt_disconnected": "MQTT disconnected",
        "container_stopped": "Container stopped",
        "container_missing": "Container not found",
...
def render_instance_card(instance):
    """Render either a full bot card or a status-only card based on connectivity."""
    bot_name = instance.get("name", "Unknown")
    docker_status = instance.get("docker_status", "unknown")
    mqtt_status = instance.get("mqtt_status", "unknown")
    health_state = instance.get("health_state", "unknown")

    if docker_status == "running" and mqtt_status == "connected" and health_state == "running":
        render_bot_card(bot_name, instance)
    else:
        render_bot_status_card(instance)
