"""
Meteora CLMM LP Guarded（Executor 模式）。
"""
...
class CLMMLPGuardedConfig(StrategyV2ConfigBase):
    ...
    trading_pair: str = Field(
        default="GAS-SOL",
        client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Trading pair (e.g. GAS-SOL): "),
    )
    ...
    target_price: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Entry trigger price (0=disable): ",
            is_updatable=True,
        ),
    )
    ...
    quote_amount: Decimal = Field(
        default=Decimal("0.2"),
        client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Fixed quote budget (e.g. SOL): ", is_updatable=True),
    )
    ...
    budget_max_wallet_pct: Decimal = Field(
        default=Decimal("0"),
        ge=0,
        client_data=ClientFieldData(prompt_on_new=False, prompt=lambda mi: "Max budget as % of wallet value (0=disable): ", is_updatable=True),
    )
    ...
    rebalance_seconds: int = Field(
        default=60,
        ge=0,
        client_data=ClientFieldData(prompt_on_new=True, prompt=lambda mi: "Out-of-range seconds before rebalance: ", is_updatable=True),
    )
    ...
    collect_fees_min_quote_value: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Collect fees min value in quote (0=disable): ",
            is_updatable=True,
        ),
    )
    rebalance_collect_fees_ratio_pct: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Rebalance fee ratio threshold % (0=disable): ",
            is_updatable=True,
        ),
    )
    rebalance_collect_fees_min_quote_value: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Rebalance fee min value in quote (0=disable): ",
            is_updatable=True,
        ),
    )
    collect_fees_to_quote: bool = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Convert collected fees to quote: ",
            is_updatable=True,
        ),
    )
    collect_fees_swap_min_quote_value: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Min fee swap value in quote (0=disable): ",
            is_updatable=True,
        ),
    )
...
class CLMMLPGuarded(StrategyV2Base):
    ...
    def format_status(self) -> str:
        ...
        lines: List[str] = []
        header = f"LP Guarded | {self.config.connector} | {self.config.trading_pair}"
        lines.append(header)
